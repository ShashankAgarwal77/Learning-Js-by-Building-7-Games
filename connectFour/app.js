document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkBoard() {
        ...
        // now take 4 values in each winningArrays and plug into squares
        for(let y = 0; y < winningArrays.length; y++) {
            ...
            else if(square1.classList.contains('player-one') &&square2.classList.contains('player-one') &&
            square3.classList.contains('player-one') &&
            square4.classList.contains('player-one')) {
                //if they do, player-two passed as winner
                result.innerHTML = 'Player Two Wins!'
            }
        }
    }

    // add  an eventListner to each square that will trigger the checkBoard function on click
